...
function saveCat() {
	...
	if (input.length > 0) { //if inputting text or selected from dropdown, 
		...
		for (var i = 0; i < categories.length; ++i) { //for each saved keyword
			if (categories[i].text === input) { //if input string matches a saved
				categories[i].sites = sitesArray; //overwrite saved sites only (no need to re-write name or id)
		...
		categories.push({"id" : newCat.id, "text" : newCat.text, "sites" : sitesArray}); //save this data to the array as new entry
...
function restoreOptions() {
	chrome.storage.sync.get({
		...
		for (var i = 0; i < categories.length; ++i) { //for each saved keyword,
			...
			newCat.text = categories[i].text; //set its text to that of the saved keyword
...
function selectedCat() {
	...
	if (cat !== "") { //if there is input in text box ...
		...
		for (var i = 0; i < categories.length; ++i) { //iterate through saved array
...
//DEBUG METHOD makes categories blank again
function clearAll() {
	categories = [];
	saveOptions();
}
